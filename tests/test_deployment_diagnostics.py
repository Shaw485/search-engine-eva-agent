...
def test_analysis_is_public_but_all_other_agent_compute_remains_owner_only() -> None:
    ...
    retrieval_owner = _nginx_location(
        nginx, "= /search-eval-api/agent/retrieval/analyze-owner"
    )
    ...
    assert 'auth_basic "Search Agent Owner";' in retrieval_owner
    _assert_owner_rate_limit(retrieval_owner)
    assert "auth_basic_user_file /etc/nginx/.search-agent.htpasswd;" in (
        retrieval_owner
    )
    assert "error_page 401 =403 /__search-agent-auth-failed;" in retrieval_owner
    assert "client_max_body_size 1k;" in retrieval_owner
    assert "limit_except POST" in retrieval_owner
    assert (
        "proxy_pass http://127.0.0.1:8010/agent/retrieval/analyze-owner;"
        in retrieval_owner
    )
    assert 'proxy_set_header Authorization "";' in retrieval_owner
    assert 'proxy_set_header Cookie "";' in retrieval_owner
    assert 'add_header Cache-Control "no-store" always;' in retrieval_owner
    assert "proxy_read_timeout 130s;" in retrieval_owner
