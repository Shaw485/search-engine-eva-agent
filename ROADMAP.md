...
| 阶段 | 周期 | 核心结果 | 状态 |
|---|---:|---|---|
| 0. 工程骨架与技术闸门 | 1–2 天 | 项目可以一条命令启动和测试 | **Not Started** |
...
| OpenSearch 环境成本过高 | 阶段 0 两天仍不稳定 | 切换本地 BM25 + FAISS 适配器 |
...
路线图通过后，只开始**阶段 0：工程骨架与技术闸门**，暂不下载全量 ESCI、暂不搭 Web 页面。阶段 0 完成时提交环境 Smoke Test、目录结构和搜索后端 ADR，再依据实测结果确认 OpenSearch 或本地适配器路线。
