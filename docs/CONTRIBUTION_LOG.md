...
| D-027 | 修复生产 Agent 工作台没有重新显示登录挑战、而直接出现 `ERR_INVALID_AUTH_CREDENTIALS` 的入口问题 | **Owner 在部署验收中主动指出**“至少要打开登录页面” | Owner 定义最低可用验收标准；**Codex 诊断**为浏览器仍使用旧 Basic Auth realm 的无效凭据缓存，并提出仅变更 realm、不变更密码或鉴权边界 | Owner 要求修复登录入口；Codex 选择 `Search Agent Owner` 作为统一的新 realm | Codex 更新全部 14 个受保护 Nginx location、部署说明和回归断言；该变更只强制浏览器重新认证，不公开工作台、不修改 htpasswd、不改变 Owner/Oracle 权限 | Owner-originated acceptance correction + Codex diagnosis/implementation |
