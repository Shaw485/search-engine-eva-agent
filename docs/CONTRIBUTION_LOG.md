...
| D-037 | Agent Plan 返回的实际模型名称/版本不再被要求与套餐别名字节级相等；只接受规范化后相同或同一别名前缀的版本化成员，并把无正文诊断细分为 status/model/output/usage/ID 错误码 | Owner 提供两次真实 Smoke 证据：旧 Key 为 `auth_failed`，新专属 Key 已通过鉴权但得到 `invalid_response` | **Codex 根据真实失败和官方 Responses“actual model name/version”契约提出并实现**同模型族匹配与安全子错误码；Owner 没有独立设计匹配算法或日志 Schema | 尚未形成新的产品政策决定；这是保持既定 Provider 选择可运行的兼容修复，仍需 Owner 再次运行真实 Smoke 验证 | Codex 更新 provider/worker、测试与 ADR；相关回归、Ruff、仓库策略和 Key 扫描通过。新 Key、响应正文和推理内容均未进入仓库或日志 | Owner-supplied validation evidence + Codex diagnosis/design/implementation; real retry pending |
