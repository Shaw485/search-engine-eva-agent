"""Build the streamable full-field ESCI catalog v2 index."""
...
from search_quality.catalog.index import EXPECTED_PRODUCT_COUNT, load_product_lock
from search_quality.catalog.index_v2 import (
...
def _execute(args: argparse.Namespace) -> None:
    ...
    metadata = build_catalog_index_v2(
        args.source,
        args.output,
    ...
    print(f"Catalog v2 index: {args.output}")


def main() -> None:
    args = build_parser().parse_args()
    configure_logging_from_args(args)
    with logging_context(trace_id=new_trace_id(), operation="catalog_v2_index_build"):
        logger.info("catalog_v2_index_command_started")
        try:
            _execute(args)
        except Exception as exc:
            logger.error(
                "catalog_v2_index_command_failed",
                extra={
