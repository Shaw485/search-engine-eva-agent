...
| D-029 | 工作台底部只展示优化前后真实发生变化的搜索样本，并同时覆盖变好与变差 | **Owner 明确提出展示与验收要求** | Owner 定义需要看见双向结果差异；**Codex 设计**跨候选只读证据投影、epsilon 边界、按 Query+方向去重、最多 10 条、方向保留和来源标注 | Owner 决定页面必须同时呈现改善与退化；没有独立选择阈值、排序或去重算法 | Codex 保持最终候选的完整门禁与指标不变，从本轮消融实验中提取非持平样本；每张卡绑定自己的 Run/Comparison/策略/门禁来源，退化样本明确标为已淘汰候选，并增加严格 API/前端合同、脱敏计数日志和回归测试 | Owner-originated product requirement + Codex-designed/implemented evidence projection |
| D-030 | Agent 工作台必须解释真实 Plan、Tool、Observation、Memory、Loop 与 Safety，并把每个动作的可审计状态转换做成无连线 Observation Inspector | **Owner 主动提出**：“解释一下目前的架构，tool, plan, memory, loop, safety等，并做好obervation可视化” | Owner 定义学习与产品可观察性目标；**Codex 设计**六格控制回路、最小披露边界、交互与独立诊断日志 | Owner 决定必须可视化并解释；没有独立设计响应字段、Memory 语义、无障碍交互或日志 Schema | Codex 基于真实 `agent_run.actions` 实现 Plan/Tool/Observation/Memory/Safety/Next 检查器，明确它不是隐藏思维链、跨 Run 学习或策略审批，并补 ADR、测试和调试说明 | Owner-originated observability requirement + Codex-designed/implemented inspector |
| D-031 | 把固定检索 Planner 升级为可选 LLM observe-decide-act loop；模型每轮只从服务端有限 option ID 中选择，Harness 裁判质量，Runtime 控制工具/预算，Owner 保留策略审批和激活权 | **Owner 主动提出**：“变成agent，基于llm，边决策边执行，加入loop，我可以给你提供大模型key” | Owner 决定项目必须进入真实 LLM 决策循环并愿意提供模型资源；**Codex 设计**聚合数据投影、option-ID 协议、可强杀 provider worker、Token/时间预算、Trace/Replay 审计、Key 边界和工作台状态/usage 可视化 | Owner 批准 LLM Agent 产品方向；尚未选择具体模型、费用预算、Prompt、option 集或发布门禁，也没有把 Key 发送到仓库/前端的决定权交给 Codex | Codex 实现可选 OpenAI Responses Planner、官方 endpoint/strict function/禁重试子进程、无 Key fail-closed、单飞 API、Run summary v2、独立模型/Provider/UI 日志、Fake LLM Runtime+Replay 测试、ADR 与 Key 注入说明；当前仅能在三个现有候选间决策，不能生成代码或新增召回/排序算法，也没有策略写权限 | Owner-originated LLM-loop decision + Codex security architecture/implementation; concrete model/cost/activation pending Owner |
| D-032 | LLM Loop 的下一条真实模型通道采用火山 Agent Plan，使用独立固定 Provider、固定北京 Region Agent Plan v3 base URL 与 Responses 路径；不开放任意 `base_url`，不把本地 changed examples 发给供应商 | **Owner 明确选择**：“火山agent plan”；此前 OpenAI Provider 用所给凭据鉴权失败，Owner 随后指定新供应商 | Owner 选择火山 Agent Plan 作为 Provider；**Codex 设计**独立 provider/key 命名、固定出站地址、Responses 适配、聚合 Observation 投影、fail-closed 配置与 OpenAI/火山隔离方案 | **Owner 决定 Provider 产品方向**；尚未独立选择具体模型 ID、费用预算或改变 Harness/审批门禁。固定 endpoint、协议适配和安全边界属于 Codex 技术方案 | Codex 编写 ADR-010，并负责 Provider 代码/测试、运行与部署/日志说明；真实 Key smoke 尚待 clean commit 后验证，Key 不进入仓库、前端、Trace、日志或本台账 | Owner-selected provider + Codex-designed implementation/security boundary; real-provider validation pending |
| D-033 | 本地火山 Key 改用 macOS 原生隐藏输入启动器，不再通过聊天、终端读入、剪贴板自动化、浏览器或 AppleScript stdout 传递 | **Owner 主动提出**：“换一个输入方式，刚才那种方式，我没办法发给你” | Owner 定义可用性问题并要求替代输入；**Codex 设计** AppKit `NSSecureTextField`、固定 `execve`、最小环境白名单、无 core dump、私有 artifact root 与独立安全日志 | Owner 选择需要新的本地输入体验；没有独立设计 AppKit、进程环境或日志契约 | Codex 实现原生 Objective-C/AppKit helper、无 Key 的编译启动壳、仓库 Ark Key 泄漏扫描、契约测试和文档；启动后只配置本地后端，不自动授权或执行 Smoke | Owner-originated usability/security requirement + Codex-designed/implemented launcher |
| D-034 | 火山 Planner 的有限选项决策显式关闭深度思考，当时保留 Pro 模型、严格函数调用与原有 Token/Runtime 门禁 | **Owner 授权真实 Smoke 与聚合数据外发**；首轮真实请求在任何 Tool 执行前命中 40 秒 Worker 超时 | Owner 决定执行真实 Provider 验证；**Codex 核对**固定 Plan Responses 方案与官方能力方向一致，但精确模型/网关兼容性仍待成功 Smoke 证明；Codex 选择将有限分类步骤设为 `thinking.type=disabled`，避免用深度推理挤占 120 秒 Agent Loop 预算 | Owner 未独立选择 thinking 参数；该参数、诊断字段与回归测试属于 Codex 技术决策 | Codex 修改火山专属请求选项并把 401 与 403 稳定错误拆开；第三次首步诊断在 7.6 秒返回 401，三次均为 0 Tool，因此应轮换/新建 Agent Plan 专用 Key 后再验证，尚不能声称模型兼容成功 | Superseded in model selection by D-035; authentication blocked |
| D-035 | macOS 火山启动器的固定模型从控制台未列出的 `doubao-seed-2.0-pro` 修正为当前 Agent Plan 套餐明确列出的 `doubao-seed-2.1-turbo` | **Codex 发现并提出**；Owner 仅要求继续完成 Agent Plan Smoke，未指定具体模型 | Owner 决定使用火山 Agent Plan 并授权同一聚合数据 Smoke；**Codex 根据 Owner 已登录控制台的非敏感配置证据**选择当前列出的 `doubao-seed-2.1-turbo`，同时保持固定 Base URL、专属 Key、关闭深度思考和原有 Safety/Harness 边界 | 模型 ID 修正是 Codex 的实现判断，不应在面试中表述为 Owner 独立选择；Owner 的决策仍是采用火山 Agent Plan 与保留人工审批 | 控制台证据显示套餐生效中、专属 API Key 已配置、Base URL 为 `/api/plan/v3`；代码与测试同步更新，真实兼容性仍须在认证成功后由 Smoke 证明 | Compatibility fix retained; real custom-backend Smoke blocked by D-036 |
| D-036 | 明确 Agent Plan 专属 API 模式：仅在 `/api/plan/v3` 使用专属 Agent Plan Key 调用，不混用普通方舟 Key 与普通 `/api/v3` 路径 | **Codex 按你新确认的官方能力信息更新了判断**；Owner 仍关注成本与安全边界 | Owner 早前授权 Agent Plan Smoke；Codex 负责保持 provider 配置与 Key 隔离，并在文档中移除不当阻塞说明；Owner 指定专属-key与模型边界为前置要求 | 该项不再是是否可直接调接口，而是由 Owner 决定是否在自研调用链中常驻该 provider 和是否允许额外计费路径 | 当前配置已与专属 API Key/端点对齐，未混用任何普通 Ark Key；真实 Smoke 是否通过仍由运行结果决定 | 资料更新为“支持专属API入口”；待真实授权运行的成功指标落库 | 
| D-037 | Agent Plan 返回的实际模型名称/版本不再被要求与套餐别名字节级相等；只接受规范化后相同或同一别名前缀的版本化成员，并把无正文诊断细分为 status/model/output/usage/ID 错误码 | Owner 提供两次真实 Smoke 证据：旧 Key 为 `auth_failed`，新专属 Key 已通过鉴权但得到 `invalid_response` | **Codex 根据真实失败和官方 Responses“actual model name/version”契约提出并实现**同模型族匹配与安全子错误码；Owner 没有独立设计匹配算法或日志 Schema | 尚未形成新的产品政策决定；这是保持既定 Provider 选择可运行的兼容修复，仍需 Owner 再次运行真实 Smoke 验证 | Codex 更新 provider/worker、测试与 ADR；相关回归、Ruff、仓库策略和 Key 扫描通过。新 Key、响应正文和推理内容均未进入仓库或日志 | Owner-supplied validation evidence + Codex diagnosis/design/implementation; real retry validated by D-038 |
